...
//        })
    ...
    override fun onBackPressed() {
        var textinput = findViewById<CustomEditText>(R.id.searchBox)
        if (textinput.text.isNullOrEmpty()) {
            val currentTime = System.currentTimeMillis()
            if (currentTime - backPressedTime < backPressedInterval) {
                // 短时间内已按下返回键两次，退出应用
                super.finishAndRemoveTask() // 或者 onBackPressed() 来关闭当前活动
            } else {
                // 显示提示
            ...
            var rsbtn = findViewById<Button>(R.id.restButton)
            rsbtn.performClick()
        }
    }
